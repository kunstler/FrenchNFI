...
Do same estimation with inla

```{r inla estimation of ir5,  message=FALSE}

# TRY WITH INLA
data.all <-rbind(tree.dead.alive.growth, tree.dead.alive.predict)
data.inla <- data.frame(log.ir5 = log(data.all$ir5),
                        logdbh = log(data.all$c13/pi),
			species =  factor(data.all$espar),
			species.plot = as.vector(unclass(factor(paste(data.all$idp,
                          data.all$espar)))),
						species.plot2 = as.vector(unclass(factor(paste(data.all$idp,
                                  data.all$espar))))
								  )
library(INLA)

if(!file.exists(file.path(output.dir, "GROWTH.INLA.rds"))){

formule_random <- log.ir5 ~ species +logdbh +logdbh:species +
           f(species.plot,  model="iid", hyper=list(theta=list(prior="loggamma",param=c(0.1,0.1)))) +
           f(species.plot2, logdbh, model="iid", hyper=list(theta=list(prior="loggamma",param=c(0.1,0.1))))
model.r.INLA <- inla(formula=formule_random,
                     data=data.inla, family="gaussian",
                     control.compute=list(dic=TRUE,
                                           config = TRUE),
                     control.inla = list(strategy = "gaussian", int.strategy = "eb"),
                     control.fixed = list( prec.intercept = 0.01),
					 verbose = TRUE)
summary(model.r.INLA)
saveRDS(model.r.INLA,file=file.path(output.dir, "GROWTH.INLA.rds"))
}else{
model.r.INLA <- readRDS(file.path(output.dir, "GROWTH.INLA.rds"))
}
# prediction
data.all$pred.ir5.inla <- exp(model.r.INLA$summary.fitted.values$mean )

data.t <-data.all[data.all$tree.num %in% tree.dead.alive.predict$tree.num,]
data.pred <- data.frame(tree.num = tree.dead.alive.predict$tree.num,
                        pred.ir5.hb = pred.g)
data.pred <- merge(data.pred, data.t[ , c('tree.num', 'pred.ir5.inla')], by = 'tree.num')
...
Compare the two type of prediction. TODO COMPARE ALSO INLA
...
plot(data.pred$pred.ir5.hb, data.pred$pred.ir5.inla,xlab="prediction HB",ylab="prediction inla ir5")
lines(0:50,0:50,col="red")
...
plot(tree.dead.alive.predict$c13,data.pred$pred.ir5.mean,xlab="c13",
 ...
 data <- data.frame(tree.dead.alive.predict$c13,data.pred$pred.ir5.inla)
 names(data) <- c("X","Y")
 colors  <- densCols(data)
plot(tree.dead.alive.predict$c13, data.pred$pred.ir5.inla,
     xlab="c13",ylab="pred ir5 inla",cex=0.2,xlim=c(0,600),ylim=c(0,80),col=colors)
 data <- data.frame(tree.dead.alive.growth$c13,tree.dead.alive.growth$ir5)
 names(data) <- c("X","Y")
 colors  <- densCols(data)
plot(tree.dead.alive.growth$c13, tree.dead.alive.growth$ir5,xlab="c13",
     ylab="prediction inla ir5",cex=0.2,xlim=c(0,600),ylim=c(0,80),col=colors)
```
...
tree.tot3$ir5.i <-  tree.tot3$ir5
tree.tot3$ir5.i[is.na(tree.tot3$ir5)] <-
tree.tot3$pred.ir5.inla[is.na(tree.tot3$ir5)]
...
tree.tot3$recruit.i[(tree.tot3$c13 -2*pi*tree.tot3$ir5.i) <23.5] <- 1
